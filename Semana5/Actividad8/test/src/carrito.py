...
class Carrito:
    ...
    def aplica_descuento_condicional(self,porcentaje,minimo):
    ...
    def obtener_items_ordernados(self,criterio:str):
        if criterio not in ['precio','nombre']:
            raise ValueError("Criterio no válido. Debe ser 'precio' o 'nombre'")
        if criterio == 'precio':
            return sorted(self.items, key=lambda x: x.producto.precio)
        else:
            return sorted(self.items, key=lambda x: x.producto.nombre)