...
def test_descuento_condicional_exitoso():
    ...
    carrito.agregar_producto(producto3, cantidad=3)
    #act
    total_antes_descuento = carrito.calcular_total()
    total_a_pagar=carrito.aplica_descuento_condicional(porcentaje=15,minimo= 500)
    #assert

    assert total_a_pagar == total_antes_descuento*0.85

def test_descuento_condicional_fallido():
    #arrage
    carrito = Carrito()
    producto1 = ProductoFactory(nombre="Laptop", precio=300.00)
    producto2 = ProductoFactory(nombre="Mouse", precio=50.00)
    producto3 = ProductoFactory(nombre="Teclado", precio=75.00)
    carrito.agregar_producto(producto1, cantidad=1)
...
def test_agregar_producto_en_stock():
    #arrage
    carrito = Carrito()
    producto1 = ProductoFactory(nombre="Laptop", precio=300.00,stock=5)

    #Act
    carrito.agregar_producto(producto1, cantidad=4)
...
def test_agregar_producto_no_en_stock():
    ...
    with pytest.raises(ValueError) as exc_info:
        carrito.agregar_producto(producto1, cantidad=6)

    #assert
    assert str(exc_info.value) == "No hay suficiente stock del producto"


def test_ordenar_lista_de_productos_criterio_definido():
    # Arrange
    carrito = Carrito()
    producto1 = ProductoFactory(nombre="Laptop", precio=1000.00)
    producto2 = ProductoFactory(nombre="Mouse", precio=50.00)
    ...
    # Act
    list_byPrice = carrito.obtener_items_ordernados(criterio="precio")
    list_byName = carrito.obtener_items_ordernados(criterio="nombre")

    # Assert
    assert [i.producto.precio for i in list_byPrice] == [50.00, 75.00, 1000.00]
    assert [i.producto.nombre for i in list_byName] == ["Laptop", "Mouse", "Teclado"]