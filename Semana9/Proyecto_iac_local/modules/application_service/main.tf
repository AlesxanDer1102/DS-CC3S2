...
data "external" "app_metadata_py" {
  program = [var.python_exe, "./scripts/python/generate_app_metadata.py"]
...
resource "null_resource" "start_service_sh" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = "${path.cwd}/scripts/bash/start_simulated_service.sh '${var.app_name}' '${local.install_path}' '${local_file.config_json.filename}'"
  }
}
